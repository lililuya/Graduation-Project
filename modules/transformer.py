...
def get_rotation_matrix(yaw, pitch, roll):
    ...
    # 修改view  因为报错 RuntimeError: view size is not compatible with input tensor's size and stride (at least one dimension spans across two contiguous subspaces). Use .reshape(...) instead.
...
class Audio2kpTransformerBBoxQDeepPrompt(nn.Module):
    ...
    def forward(self, x, initial_kp = None, return_strg=False, emoprompt=None, deepprompt=None, hp=None, side=False):
        ...
        for i in range(bbs):
            ...
            # print(initial_kp["prediction_map"][i].shape)  # torch.Size([15, 16, 128, 96])  # torch.Size([15, 16, 64, 64])
            face_map = initial_kp["prediction_map"][i].reshape(15*16, 64*64).permute(1, 0).reshape(64*64, 15*16)
