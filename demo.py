...
def prepare_test_data(img_path, audio_path, opt, emotype, use_otherimg=True):
    ...
    # driving latent
    # latent_path_driving = f'{root_wav}/latent_evp_25/{asp}.npy'             # 驱动的latent
    latent_path_driving = "/mnt/sdb/cxh/liwen/EAT_code/preprocess/latents/mytemplate.npy"
    # pose_gz = gzip.GzipFile(f'{root_wav}/poseimg/{asp}.npy.gz', 'r')        # 一个pose模版
    pose_gz = gzip.GzipFile("/mnt/sdb/cxh/liwen/EAT_code/preprocess/poseimg/mytemplate.npy.gz")
    poseimg = np.load(pose_gz)                                              
    
    
    # deepfeature = np.load(f'{root_wav}/deepfeature32/{asp}.npy')
    deepfeature = np.load("/mnt/sdb/cxh/liwen/EAT_code/demo/video_processed/obama/deepfeature32/obama.npy")
    print(deepfeature.shape)
...
from sync_batchnorm import DataParallelWithCallback  # batchnorm干啥

"""
kp_detector和he_estimator权重加载
"""
...
def test(ckpt, emotype, save_dir=""):
    ...
    all_wavs2 = ["/mnt/sdb/cxh/liwen/EAT_code/demo/test_audio_original/temp.wav"]
...
if __name__ == '__main__':
    ...
    argparser.add_argument("--save_dir", type=str, default="/mnt/sdb/cxh/liwen/EAT_code/result_video", help="path of the output video")
