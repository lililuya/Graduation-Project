...
def conv_audios_to_deepspeech(audios,
    ...
    with tf.compat.v1.Session(graph=graph, config=config) as sess:
        for audio_file_path, num_frames in zip(audios, num_frames_info):
    ...
    deep_feature.append(np.array(windows))
            # np.save("/mnt/sdb/cxh/liwen/EAT_code/demo/test_deepspeech/test.npy", np.array(windows))
    
    if len(deep_feature) == 1:
        return deep_feature[0]
    else:
        print("more than 2 audio in dir")
        exit()
...
def prepare_deepspeech_net(deepspeech_pb_path):
    ...
    end_time = time.time()
    print(f"<==================== End Initiating deepspeech model====================>")
