...
class metahuman():
    ...
    #         save_folder = "/mnt/sdb/cxh/liwen/EAT_code/demo/test_asr/ZH"
    ...
    #     if not isinstance(audio_in_path, str):
    #         save_folder = "/mnt/sdb/cxh/liwen/EAT_code/demo/test_asr/EN"
    #         save_file   = os.path.join(save_folder, "asr_audio.wav")
    #         samplerate, data = file_obj
    #         sf.write(save_file, data, samplerate = samplerate)  # 假设采样率为 44100
    #         print("音频文件已保存到:", save_file)
    ...
    def ASR_Linguage(self, audio_in_path, language):
        ...
        """
        if not isinstance(audio_in_path, str):
            save_folder = "/mnt/sdb/cxh/liwen/EAT_code/audio_file/audio_asr_test"  # 指定存放测试音频路径文件
    ...
    # abs_wavpath得是一个具体的路径才行
    def specialTTS(self, input_text, language, abs_wavpath = None):
        now = datetime.now()
        formatted_now = now.strftime("%Y-%m-%d-%H-%M-%S")
        # 根据abs的path判断即可，如果是来自User的测试数据，路径的父目录肯定是audio_tts,如果传过来是空值，重新给个路径
        if abs_wavpath is None:  # 说明test传过来的
            language_mapping = lambda language: "zh" if language == "中文" else "en"
            audio_name = f"{language_mapping}_{formatted_now}_test.wav"
            abs_wavpath = os.path.join("/mnt/sdb/cxh/liwen/EAT_code/audio_file/audio_tts_test", audio_name) # 写入配置文件
            
        if language == "英文":
            audio_tts_en = self.gpt_infer_model_en.get_tts_wav(input_text, language)
            sf.write(abs_wavpath, audio_tts_en, self.sample_rate, 'PCM_24')  # 写入指定路径
            return abs_wavpath
        elif language == "中文":
    ...
    def generate_audio(self, audio_in_path, tts_audio_path, language_type):
        start_time  = time.time()
        # 对语言进行判断定
        if language_type== "中文":
        ...
        audio_tts   = self.specialTTS(chat_input, language_type, abs_wavpath)
    ...
    def write_images_restoration(self, result_list, res_path="/mnt/sdb/cxh/liwen/EAT_code/save_videos/restoration"):
    ...
    def judge_pose(self, pose):
        if pose=="驱动动作1":
            pose_path = "/mnt/sdb/cxh/liwen/EAT_code/preprocess/poseimg/mytemplate.npy.gz"
            return pose_path
        elif pose=="驱动动作2":
            pose_path = "/mnt/sdb/cxh/liwen/EAT_code/demo/video_processed/obama/poseimg/obama.npy.gz"
            return pose_path
        elif pose=="驱动动作3":
            pose_path = "/mnt/sdb/cxh/liwen/EAT_code/demo/video_processed/W015_neu_1_002/poseimg/W015_neu_1_002.npy.gz"
            return pose_path
    ...
    """
    @torch.no_grad()
    def __call__(self, img, emotype, audio_file, language_type, pose, tts_audio_path="/mnt/sdb/cxh/liwen/EAT_code/audio_file/audio_tts"):
        pose_path = self.judge_pose(pose)
        save_folder = "/mnt/sdb/cxh/liwen/EAT_code/audio_file/audio_asr"
        save_file  = self.save_audio(audio_file, save_folder)  # 保存输入到ASR的音频文件到指定的目录audio_path
        audio_path = save_file
        save_video_path = "/mnt/sdb/cxh/liwen/EAT_code/save_videos"  # 拿到配置里面去
        restoration_video_path = "/mnt/sdb/cxh/liwen/EAT_code/save_videos/restoration"
        emotype = self.transfer_emotype(emotype)
        start_time  = time.time()
        abs_wavpath, chat_input = self.generate_audio(audio_path, tts_audio_path, language_type)
        ...
        # abs_wavpath = "/mnt/sdb/cxh/liwen/EAT_code/demo/test_wav/output.wav"
